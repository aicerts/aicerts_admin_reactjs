...
import { Table, Modal, Button as BootstrapButton, Form, Spinner } from 'react-bootstrap'; // Import Spinner for loading
...
const ServerTable = ({ onView }) => {
    ...
    const [alertModal, setAlertModal] = useState({ show: false, errorMessage: '', successMessage: "" }); // State for Alert Modal
    const [formData, setFormData] = useState({ serverName: '', serverAddress: '', endPoint: '' });
                            ...
                            <td>
                                <div
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        backgroundColor: serverStatuses[server.serverAddress] === 'Active' ? 'rgba(255, 136, 91, 0.1)' : 'rgba(250, 188, 63, 0.1)',
                                        padding: '2px 6px',
                                        borderRadius: '4px',
                                        width: 'fit-content',
                                        color: serverStatuses[server.serverAddress] === 'Active' ? '#FF885B' : '#DB371F',
                                    }}
                                >
                                    <span
                                        style={{
                                            ...
                                            height: '10px',
                                            borderRadius: '50%',
                                            backgroundColor: serverStatuses[server.serverAddress] === 'Active' ? 'rgba(255, 136, 91, 1)' : 'rgba(219, 55, 31, 1)',
                                            marginRight: '5px',
                                        }}
                                    />
    ...
    onClick={!loadingDelete ? () => handleDelete(server.serverName) : null} 
>
    <Image 
        src={Recycle} 
        alt='remove' 
